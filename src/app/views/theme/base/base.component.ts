...
import { LayoutConfigModel, LayoutConfigService, MenuConfigService, PageConfigService } from '../../../core/_base/layout';
...
export class BaseComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		const subscr = this.layoutConfigService.onConfigUpdated$
		.subscribe((layoutConfig: LayoutConfigModel) => {
	...
	ngOnInit(): void {
		...
		const subscr = this.layoutConfigService.onConfigUpdated$.subscribe((cfg: object) => {
			setTimeout(() => {
	...
	loadRolesWithPermissions() {
		...
		const subscr = this.currentUserPermissions$.subscribe((res: Permission[]) => {
			if (!res || res.length === 0) {
				return;
			}
